...
function CurrencyCell({
  ...
  const save = (options?: { useFallbackOnEmpty?: boolean }) => {
    ...
    const parsedValue = parseNumberEs(isEmpty ? '' : normalizedText);
    const nextValue =
                  ...
                  <td>
                    {(r.decrement ?? 0) > 0 ? (
                      <div>
                        ...
                        {r.decrementReturnPct !== undefined && (
                          <small style={{ display: 'block', marginTop: 4, color: '#16a34a', whiteSpace: 'nowrap' }}>
                            Genera {formatCurrency((r.decrement ?? 0) * r.decrementReturnPct)}
                          </small>
                        )}
                      </div>
                    ) : (
...
export default function App() {
  ...
  const firstActiveMonthByClient = useMemo(() => {
    const result: Record<string, string | undefined> = {};

    pendingEligibleClientIds.forEach((clientId) => {
      const movementMonths = Object.entries(movementsByClient[clientId] ?? {})
        .filter(([, movement]) =>
          movement.increment !== undefined ||
          movement.decrement !== undefined ||
          movement.manualProfit !== undefined
        )
        .map(([iso]) => iso.slice(0, 7));

      const historyMonths = Object.entries(monthlyHistoryByClient[clientId] ?? {})
        .filter(([, entry]) => hasMonthlyHistoryValue(entry))
        .map(([month]) => month);

      const months = [...movementMonths, ...historyMonths].sort();
      result[clientId] = months[0];
    });

    return result;
  }, [movementsByClient, monthlyHistoryByClient, pendingEligibleClientIds]);
    ...
    return trackedMonths
      .map((month) => ({
        ...
        missingClientIds: pendingEligibleClientIds
          .filter((clientId) => {
            const firstActiveMonth = firstActiveMonthByClient[clientId];
            if (!firstActiveMonth) return false;
            if (month <= firstActiveMonth) return false;
            return !hasMonthlyHistoryValue(monthlyHistoryByClient[clientId]?.[month]);
          })
      }))
      .filter((entry) => entry.missingClientIds.length > 0);
  }, [allHistoryMonths, monthlyHistoryByClient, pendingEligibleClientIds, firstActiveMonthByClient]);
  ...
  const missingClientIdsForBulkMonth = useMemo(
    () =>
      pendingEligibleClientIds.filter((clientId) => {
        const firstActiveMonth = firstActiveMonthByClient[clientId];
        if (!firstActiveMonth) return false;
        if (bulkMonthlyMonth <= firstActiveMonth) return false;
        return !hasMonthlyHistoryValue(monthlyHistoryByClient[clientId]?.[bulkMonthlyMonth]);
      }),
    [bulkMonthlyMonth, monthlyHistoryByClient, pendingEligibleClientIds, firstActiveMonthByClient]
  );
...
function InfoClientes({
  ...
  const waitlistTotalAmount = useMemo(
    () => waitlistClients.reduce((sum, w) => sum + w.amount, 0),
    [waitlistClients]
  );
  ...
  const topGuarantees = useMemo(
    ...
    [contacts, guarantees, snapshot.clientRowsById]
  );
  const generalMovementHistory = useMemo(
    () => [...snapshot.dailyRows]
      .filter((r) => (r.increments ?? 0) !== 0 || (r.decrements ?? 0) !== 0)
      .slice(-12)
      .reverse()
      .map((r) => ({
        iso: r.iso,
        ...
        increment: r.increments ?? 0,
        decrement: r.decrements ?? 0,
        net: (r.increments ?? 0) - (r.decrements ?? 0)
      })),
    [snapshot.dailyRows]
  );
    ...
    <div className="info-clients-container fade-in">
      ...
      <aside className="info-sidebar">
        ...
        <div className="info-list">
          ...
          >
            <span className="info-name">Vista general</span>
      ...
      <main className="info-detail">
        ...
        <section className="info-section">
          ...
          <div className="info-form">
            ...
            <label>
              <span>Interes de ampliacion (€)</span>
              <input
            ...
            <label style={{ gridColumn: '1 / -1' }}>
              <span>Detalle de ampliacion</span>
              <input
                id="contact-waitlist-note"
                name="contact-waitlist-note"
                placeholder="Ej: Disponible para entrar cuando se libere capacidad"
        ...
        <section className="info-section">
          <h4>{isGeneralView ? 'Panel general de clientes' : 'Resumen global inmediato'}</h4>
          <div className="info-checklist">
            ...
            <div className={clsx('info-check', waitlistTotalAmount > 0 ? 'pending' : 'ok')}>
              <span>{waitlistTotalAmount > 0 ? 'Pendiente' : 'OK'}</span>
              <strong>Capital solicitado: {formatCurrency(waitlistTotalAmount)}</strong>
            </div>
            {isGeneralView && (
              <>
                <div className="info-check ok">
                ...
                <div className="info-check ok">
                  <span>Base</span>
                  <strong>Con email: {clientsWithEmail}</strong>
                </div>
                <div className="info-check ok">
                  <span>Base</span>
                  <strong>Con telefono: {clientsWithPhone}</strong>
                </div>
              </>
            )}
          </div>
          <div className="data-table compact" style={{ marginTop: 12 }}>
            <div className="table-header" style={{ gridTemplateColumns: '1fr 160px 1fr' }}>
              <div>Cliente</div>
              <div>Capital solicitado</div>
              <div>Detalle</div>
            </div>
            {waitlistClients.length === 0 && <div className="table-row"><div>No hay solicitudes de ampliacion</div></div>}
          ...
          {isGeneralView && (
            <>
              <div className="data-table compact" style={{ marginTop: 12 }}>
                <div className="table-header" style={{ gridTemplateColumns: '1fr 180px' }}>
                  <div>Top garantias actuales</div>
                  <div>Garantia actual</div>
                </div>
                {topGuarantees.length === 0 && <div className="table-row"><div>Sin datos</div></div>}
                {topGuarantees.map((g) => (
                  <div key={g.id} className="table-row" style={{ gridTemplateColumns: '1fr 180px' }}>
                    <div>{g.name}</div>
                    <div>{formatCurrency(g.value)}</div>
                  </div>
                ))}
              </div>
              <div className="data-table compact" style={{ marginTop: 12 }}>
                ...
                {generalMovementHistory.length === 0 && <div className="table-row"><div>Sin movimientos globales</div></div>}
                {generalMovementHistory.map((m) => (
